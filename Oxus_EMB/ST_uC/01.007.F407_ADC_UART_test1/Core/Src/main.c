...
/*
 * Project is Polling ADC multichannel read(IN8,IN9)
 ...
 * Scan Conversion Mode: Enabled) because it puts a channel read into queue and
 * reads after HAL_ADC_PollForConversion(&hadc1, 1000);
 *
 * */
#include <stdio.h>
#include <string.h>
...
//  HAL_ADCEX_CALIBRATION_START(&HADC1,ADC_SINGLE_ENDED);	//The function is NOT available on STM32F4 series! This function only exists on newer STM32 families like F3, L4, G4, H7, etc.
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  uint16_t ch8 = HAL_ADC_GetValue(&hadc1);  // PB0

	  HAL_ADC_PollForConversion(&hadc1, 1000);
	  uint16_t ch9 = HAL_ADC_GetValue(&hadc1);  // PB1

	  HAL_ADC_Stop(&hadc1);


	  DebugPrint("CH8: %d | %.3f V, CH9: %d | %.3f V\r\n",ch8, adc_to_voltage(ch8), ch9, adc_to_voltage(ch9));
