...
/*
 * Project is Interrupt ADC multichannel read(IN8,IN9)
 ...
 * Scan Conversion Mode: Enabled, Continous Conversion Mode: Enabled).
 * void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc) entered after each
 * conversion. HAL_ADC_Start_IT(&hadc1); should be inside the while loop. I did
 * not work before while loop. Entered 1 time, did not enter continuously as referred
 * in "Steppe School"
 *
 * while (1)
  {
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
	...
	if(counter == 0)
	{
		IN8_PB0 = HAL_ADC_GetValue(&hadc1);
		DebugPrint("CH8: %d | %.3f V\r\n",IN8_PB0, adc_to_voltage(IN8_PB0));
		counter = 1;

	}
	else
	{
		IN9_PB1 = HAL_ADC_GetValue(&hadc1);
		DebugPrint("CH9: %d | %.3f V\r\n",IN9_PB1, adc_to_voltage(IN9_PB1));
		counter = 0;
	}
