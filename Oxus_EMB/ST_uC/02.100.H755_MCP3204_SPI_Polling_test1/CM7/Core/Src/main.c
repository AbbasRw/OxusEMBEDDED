...
uint16_t sensor_read_mcp3208(void)
{
	uint8_t addr = (0b00011000 | (0b010 & 0b111));
	uint8_t b1, b2;

	HAL_GPIO_WritePin(GPIOA,GPIO_PIN_4,GPIO_PIN_RESET);

	(void)spi_transfer(&hspi1, addr);
	b1 = spi_transfer(&hspi1, 0);
	b2 = spi_transfer(&hspi1, 0);

	HAL_GPIO_WritePin(GPIOA,GPIO_PIN_4,GPIO_PIN_SET);

	// 12-bit ADC değeri hesaplanıyor.
	uint16_t adcValue = ((b1 & 0x3F) << 6) | ((b2 & 0xFC) >> 2);
...
/* USER CODE END Boot_Mode_Sequence_2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  	  adc_val = sensor_read_mcp3208();
	  	  adc_vol = (adc_val/4096.0f) * 4.096f;
	  	  check_dma_data();
	  	  DebugPrint("ADC value: 0x%x | Voltage: %.3fV\r\n",adc_val, adc_vol);
	  	  HAL_Delay(1000);
  }
  /* USER CODE END 3 */
}
