...
uint8_t nextion_rx_buffer[128];
uint16_t nextion_rx_index = 0;

//#define UART6_DMA_RX_BUF_SIZE 128
//
//UART_HandleTypeDef huart6;
//
///* DMA RX buffer */
//uint8_t uart6_dma_rx_buf[UART6_DMA_RX_BUF_SIZE];
//
///* Software read index */
//uint16_t uart6_rx_old_pos = 0;
/* USER CODE END PV */
...
//void Nextion_DMA_Process(void)
//{
//    uint16_t new_pos;
//    uint16_t i;
//
//    /* Calculate current DMA write position */
//    new_pos = UART6_DMA_RX_BUF_SIZE -
//              __HAL_DMA_GET_COUNTER(huart6.hdmarx);
//
//    /* If new data arrived */
//    if (new_pos != uart6_rx_old_pos)
//    {
//        if (new_pos > uart6_rx_old_pos)
//        {
//            /* No wrap-around */
//            for (i = uart6_rx_old_pos; i < new_pos; i++)
//            {
//                DebugPrint("%02X ", uart6_dma_rx_buf[i]);
//            }
//        }
//        else
//        {
//            /* Wrap-around case */
//            for (i = uart6_rx_old_pos; i < UART6_DMA_RX_BUF_SIZE; i++)
//            {
//                DebugPrint("%02X ", uart6_dma_rx_buf[i]);
//            }
//            for (i = 0; i < new_pos; i++)
...
//        uart6_rx_old_pos = new_pos;
//    }
//}



void Nextion_Poll_And_Forward(void)
{
    if (HAL_UART_Receive(&huart6, &nextion_rx_byte, 1, 1) == HAL_OK)
    {
        char buf[8];

        if (nextion_rx_index < sizeof(nextion_rx_buffer))
        {
            nextion_rx_buffer[nextion_rx_index++] = nextion_rx_byte;
        }

        sprintf(buf, "%02X ", (unsigned int)nextion_rx_byte);
        DebugPrint(buf);
    }
}
...
/* USER CODE END Boot_Mode_Sequence_2 */
  ...
  MX_USART6_UART_Init();
  /* USER CODE BEGIN 2 */
  DebugPrint("UART3 DMA started! Type something...\r\n");
