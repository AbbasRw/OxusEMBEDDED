...
volatile TMC_State_t tmcState = TMC_IDLE;
uint32_t tmcTimestamp;
...
void parse_tmc_response(uint8_t *data)
{
	DebugPrint("TMC: ");
    for (int i = 0; i < 8; i++)
...
uint8_t tmc2208_crc(uint8_t *data, uint8_t len)
{
    uint8_t crc = 0;
...
void build_tmc_frame(uint8_t *buf)
{
    buf[0] = 0x05;     // SYNC
    buf[1] = 0x00;     // Slave address
    buf[2] = 0x00;     // GCONF
    buf[3] = 0x00;
    buf[4] = 0x00;
    buf[5] = 0x00;
    ...
    buf[7] = 0x07;
//    buf[7] = tmc2208_crc(buf, 7);
}


void TMC2208_StartRead(void)
{
    if (tmcState == TMC_IDLE)
    {
        build_tmc_frame(tmc_tx);
        tmcState = TMC_TX;
    }
}
...
/* USER CODE END Boot_Mode_Sequence_2 */
  ...
  uint8_t test[7] = {0x05,0x00,0x00,0x00,0x00,0x00,0x00};
  uint8_t crc = tmc2208_crc(test, 7);
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
    	DebugPrint("A\n\r");
        tmcState = TMC_WAIT_TURNAROUND;
        tmcTimestamp = HAL_GetTick();
    }
}
