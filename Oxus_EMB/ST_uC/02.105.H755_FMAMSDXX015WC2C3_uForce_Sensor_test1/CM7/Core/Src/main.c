...
/* I2C address */
#define MICROFORCE_I2C_ADDR   (0x28 << 1)

/* ===== SENSOR CONFIGURATION ===== */
/* Adjust to your exact sensor variant */
#define RATED_FORCE_N   15.0f     // datasheet 15 N sensor

/* ===== ADC CONSTANTS ===== */
#define ADC_COUNTS_MAX  16383.0f
#define OUTPUT_MIN      (0.2f * ADC_COUNTS_MAX)
#define OUTPUT_MAX      (0.8f * ADC_COUNTS_MAX)

HAL_StatusTypeDef MicroForce_Read(MicroForce_Data_t *data)
{
    uint8_t rx[4];
    ...
    /* 11-bit temperature output */
    uint16_t raw_temp =
        ((uint16_t)rx[2] << 3) | (rx[3] >> 5);

    /* ===== FORCE CONVERSION (Equation 2) ===== */
    data->force_n =
        ((raw_force - OUTPUT_MIN) * RATED_FORCE_N) /
        (OUTPUT_MAX - OUTPUT_MIN);

    /* ===== TEMPERATURE CONVERSION (Equation 3) ===== */
    data->temperature_c =
        ((float)raw_temp * 200.0f / 2047.0f) - 50.0f;
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if (HAL_GetTick() - last >= 500)
	  {
		  ...
		  if (MicroForce_Read(&mf) == HAL_OK)
		  {
		      DebugPrint("Force: %.3f N | Temp: %.2f C\r\n",
		                 mf.force_n,
		                 mf.temperature_c);
		  }
		  else
		  {
		      DebugPrint("Sensor read error\r\n");
		  }
