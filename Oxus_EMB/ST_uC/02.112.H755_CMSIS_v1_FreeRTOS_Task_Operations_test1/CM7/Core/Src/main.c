...
void Task2_init(void const * argument)
{
  ...
  for(;;)
  {
    DebugPrint("Task2, index = %d\n\r",indx++);
//	HAL_GPIO_TogglePin(YELLOW_LED_GPIO_Port, YELLOW_LED_Pin);
    osDelay(2000);

    if(indx == 4)
    {
    	DebugPrint("Default SUSPENDING\n\r");
    	osThreadSuspend(defaultTaskHandle);
    }

    if(indx == 8)
    {
    	DebugPrint("Default RESUMING\n\r");
    	osThreadResume(defaultTaskHandle);
    }

//    if(indx == 20)
//    {
//    	DebugPrint("Default TERMINATING\n\r");
//    	osThreadTerminate(defaultTaskHandle);
//    }
//
//    if(indx == 25)
//    {
//    	DebugPrint("Self TERMINATING\n\r");
...
//    }
    ...
    if(indx == 15)
    {
    	...
    	DebugPrint("5 second osDelayUntil\n\r");
